...
namespace astroaccelerate {
  ...
  private:
    ...
    //float              nsamp_local;   // added by abhinav //new edits
    int                count_bf_count; // added by abhinav //new edits
...
//      } // if strat condition ends here ------------------------------------- new edits
    ...
    bool run_pipeline(const bool dump_to_disk, const bool dump_to_user, std::vector<analysis_output> &user_output, aa_pipeline_runner::status &status_code) {
      ...
      if(t >= num_tchunks) {
        count_bf_count = count_bf_count + 1; //new edits
        printf("\n count_bf_count :: %d \n", count_bf_count); //new edits
...
// more edits - abhinav ---------------------------------------------------------
      t = 0;
      inc = 0;
      tstart_local=0; //Added by Rushikesh
...
//printf("for debug: obf for loop over");
	if(dump_to_user) {
//	printf("for debug: dump_to_user if conditional over");
...
//printf("for debug: save data over");	
	//Add analysis
...
//printf("for debug: vector initialzed");
...
printf("for debug: inside analysis");
	if(dump_to_user) {
