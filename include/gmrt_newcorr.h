...
extern "C"
{
  ...
  */
  ...
  enum
  {
    ...
    BEDAS_CARDS,
    GPUDAS_CARDS
  }; /* FishCamp Data Acquisition Cards jnc 15/Apr/05 */
  enum
  {
    IndianPolar,
    UsbPolar,
  ...
  typedef struct
  {
    float phase, dp, delay, dd /* sec/sec */;
  } ModelParType;
  typedef struct
  {
  ...
  */
  ...
  {
    int BBRFIFilterFlag;
    char ReplacementType[64]; // constant, digital noise, or zero;
    int ReplaceValue;         // contstan, zero, digital_noise_sigma
    float Threshold;          // n-sigma
    char Strategy[32];        // voltage or  power
    char param1[64];          // for future provision.
    char param2[64];          // for future provision.
    char param3[64];          // for future provision.
  } BBRFIFilterType;
  ...
  typedef struct
  {
    ...
    float RA[4];     // for future provision
    float DEC[4];    // for future provision
    char param2[64]; // for future provision
    char param3[64]; // for future provision
    char param4[64]; // for future provision
  } BeamSteeringType;

  typedef struct
  {
    char Feature1Title[64]; // for future provision
    int Feature1Flag;
    char param1[64]; // for future provision
    char param2[64]; // for future provision
    char param3[64]; // for future provision
  ...
  typedef struct
  {
    char Feature2Title[64]; // for future provision
    int Feature2Flag;
    char param1[64]; // for future provision
    char param2[64]; // for future provision
    char param3[64]; // for future provision
  ...
  typedef struct
  {
    char Feature3Title[64]; // for future provision
    int Feature3Flag;
    char param1[64]; // for future provision
    char param2[64]; // for future provision
    char param3[64]; // for future provision
  ...
  typedef struct
  {
    char Feature4Title[64]; // for future provision
    int Feature4Flag;
    char param1[64]; // for future provision
    char param2[64]; // for future provision
    char param3[64]; // for future provision
  ...
  typedef struct
  {
    int PFBFlag;     // 0/1/2 no de-Dispersion, incoherent, coherent dispersion
    int Taps;        // 0/1/2 no de-Dispersion, incoherent, coherent dispersion
    char param1[64]; // for future provision
    char param2[64]; // for future provision
    char param3[64]; // for future provision
  } PFBType;
  ...
  typedef struct
  {
    int CDFlag;                // 0/1/2 no de-Dispersion, incoherent, coherent dispersion
    unsigned int in_channels;  // 0/1/2 no de-Dispersion, incoherent, coherent dispersion
    unsigned int out_subbands; // 0/1/2 no de-Dispersion, incoherent, coherent dispersion
    float freq;                // 0/1/2 no de-Dispersion, incoherent, coherent dispersion
    float bw;                  // 0/1/2 no de-Dispersion, incoherent, coherent dispersion
    float DM;                  // 0/1/2 no de-Dispersion, incoherent, coherent dispersion
    int sb_flag;               // 0/1/2 no de-Dispersion, incoherent, coherent dispersion
    int vlt_bits;              // 0/1/2 no de-Dispersion, incoherent, coherent dispersion
    int write_int;             // 0/1/2 no de-Dispersion, incoherent, coherent dispersion
    float sampling_resolution; // 0/1/2 no de-Dispersion, incoherent, coherent dispersion
  } CDType;
  ...
  typedef struct
  {
    ...
    unsigned long GAC_maskFp[MAX_BEAMS]; // GAC MASK pol2
    ...
    // New header information/structure added to 2.7 version.
    PFBType PFBparams;
  ...
  {
    short int SyncCmd;
    short int VersionMajor; // Version major
    short int VersionMinor; // Version minor
...
// # define DAS_BUFSIZE 138412032 // 132 MB for GUP corr 32 ant, 4 stokes 2048 chan 4 buffs
...
#define SAMP_CLK 0.000000005
  ...
  */
  ...
  // void calModel(double tm, CorrType *corr, ModelParType *par, int scans,ScanInfoType *scaninfo, AntennaFlagType *antflag, delay_vals_t * delays, fringe_vals_t * fringes);
  // void calModelPar(double tm, CorrType *corr, ModelParType *mpar, SourceParType *source, int *antmask, int scan, delay_vals_t * delays, fringe_vals_t * fringes);
